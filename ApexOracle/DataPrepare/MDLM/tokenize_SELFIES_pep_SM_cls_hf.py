...
CSV_PATH = current_dir/'Data'/'selfies_pep_SM_cls_splits_v2'/f'part_{args.split}.csv'
...
def process_chunk(chunk):
    ...
    for _, row in chunk.iterrows():
        try:
            ...
            # try:
            #     matches = mol.GetSubstructMatches(global_pattern)
            ...
            #         label = 0
            # except:
            #     chunk_stats['failed_judgement'] += 1
            #     continue

            # 保存有效数据
            chunk_stats['raw_tokens'].append((row['ID'], input_ids, row['label']))
            # chunk_stats['labels'].append(label)
...
if __name__ == "__main__":
    ...
    full_df = pd.read_csv(
        CSV_PATH,
        dtype={'ID': 'string', 'SELFIES': 'string', 'label': 'int'}
    )
    ...
    with Pool(processes=N_WORKERS) as pool:  # TODO: descriptor removed
    ...
    if valid_samples > 0:
        ...
        mol_ids = [t[0] for t in final_stats['raw_tokens']]
        labels = [t[2] for t in final_stats['raw_tokens']]
        ...
        dataset = Dataset.from_dict({
            'mol_ids': mol_ids,
            'input_ids': input_ids,
            'labels': labels # TODO: descriptor removed
        })

        # 保存数据集
        print('saving dataset')
        # output_filename = f"dataset_{source_name}"
        output_path = current_dir/'Data'/'selfies_pep_SM_cls_hf_db_shards_v2'/f'shard_{args.split}'
        ...
        print(f"label 示例: {labels[0]}...") # TODO: descriptor removed
