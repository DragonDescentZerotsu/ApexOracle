...
RAW_PATH   = "/data1/fangping2/hf_pep_SM_cls_v2"
PADDED_PATH = "/data1/fangping2/hf_pep_SM_cls_1024_v2"
NUM_PROC   = 210
MAX_LEN = 1024
...
def _project(example):
    ...
    L = len(ids)
    # attention mask：前 L 个是真实 token，后面都是 0
    attention_mask = [1] * L + [0] * (MAX_LEN - L)
    # pad input_ids
...
filtered.save_to_disk(PADDED_PATH)
print(f"过滤后数据集已保存至：{PADDED_PATH}")