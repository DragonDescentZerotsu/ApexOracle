...
CSV_PATH = "/data1/tianang/Projects/Synergy/DataPrepare/Data/inhouse_DBAASP_id_SMILES_merged.csv"
source_name = CSV_PATH.split('.csv')[0].split('/')[-1].rsplit('_', 1)[0]
CHUNKSIZE = 100000  # 每个分块处理10万行
N_WORKERS = max(1, cpu_count() - 9)  # 保留2个核心给系统
    ...
    for _, row in chunk.iterrows():
        try:
            ...
            if unk_count > 0:
                ...
                chunk_stats['id_w_unk'].append(row['DBAASP_id'])

            chunk_stats['lengths'].append(len(tokens))
        except Exception as e:
            print(f"Error processing {row['DBAASP_id']}: {str(e)}")
...
if __name__ == "__main__":
    ...
    with Pool(processes=N_WORKERS) as pool:
        ...
        reader = pd.read_csv(
            CSV_PATH,
            header=None,
            names=['DBAASP_id', 'SMILES'],
            chunksize=CHUNKSIZE,
            dtype={'DBAASP_id': 'string', 'SMILES': 'string'}
        )
    ...
    print(f"包含UNK的SMILES数量: {final_stats['unk_smiles']}")
    ...
    plt.savefig(
        f'/data1/tianang/Projects/Synergy/DataPrepare/MDLM/Data/token_length_distribution_{source_name}_{MODEL_NAME.split('/')[-1]}.pdf',
        bbox_inches='tight',
        dpi=300,
