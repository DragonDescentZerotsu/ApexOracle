...
def main(smiles_list):
    ...
    with mp.Pool(processes=128) as pool:
        fingerprints = pool.map(compute_fingerprint, smiles_list)
    ...
    with mp.Pool(processes=128) as pool:
        for i, j, sim in tqdm(pool.imap_unordered(compute_similarity_pair, tasks), desc='Comparing Morgan Fingerprints', total=len(tasks)):
            similarity_matrix[i, j] = sim
            similarity_matrix[j, i] = sim  # 由于相似性矩阵是对称的
