...
The non-release `virus-extension` development line adds `ApexOracle-Virus` as a sixth active gitlink while retaining
the five paper modules. Its machine-readable lock is authoritative for that branch; the addition does not alter the
immutable five-module `v0.2.3` release. Core and MDLM maintenance are merged/fast-forwarded into the extension line,
and the six-module source archive and repository policy are validated independently from the historical release.
...
GitHub Release archive is the canonical way to download one source-only tree containing all five fixed paper modules;
